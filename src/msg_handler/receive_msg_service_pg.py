...
"""
    entrypoint for the message queue handler
...
def run():
    """
    fires up the message handler to process ASGS run status updates
    ...
    # create a logger
    logger = LoggingUtil.init_logging("APSVIZ.APSViz-Msg-Handler.receive_msg_service", level=log_level, line_format='medium', log_file_path=log_path)
    ...
    try:
        ...
        logger.exception("FAILURE - Problems initiating receive_msg_queue.")
