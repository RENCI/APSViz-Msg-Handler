...
def run():
    ...
    try:
        ...
        channel.queue_declare(queue='rp_queue')

        logger.info("ecflow_run_props_msg_svc channel and queue declared.")

        # get an instance to the callback handler
        queue_callback_inst = AsgsQueueCallback(_logger=logger)

        channel.basic_consume('rp_queue', queue_callback_inst.ecflow_run_props_callback, auto_ack=True)

        logger.info('ecflow_run_props_msg_svc configured and waiting for messages...')
