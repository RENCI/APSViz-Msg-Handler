...
class AsgsConstants:
    ...
    def get_lu_id_from_msg(self, msg_obj, param_name, lu_name):
        ...
        # get the ID
        ret_id = self.get_lu_id(ret_name, lu_name)

        # did we find something
        if ret_id >= 0:
            self.logger.debug("PASS - LU name: %s, Param name: %s, ID: %s", lu_name, param_name, str(ret_id))
        else:
            self.logger.error("FAILURE - Invalid or no param name: %s not found in: %s", param_name, lu_name)

        # return to the caller
        return ret_id, ret_name

    def get_lu_id(self, element_name, lu_name):
        """
        ...
        # did we find something
        if ret_id >= 0:
            self.logger.debug("PASS - LU name: %s, element name: %s, ID: %s", lu_name, element_name, str(ret_id))
        else:
            self.logger.error("FAILURE - Invalid or no element name: %s not found in: %s", element_name, lu_name)
